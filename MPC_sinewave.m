...
figure ;
    plot(xOpt_full(:,49),xOpt_full(:,50)); hold on;
    plot(xOpt_full(:,56),xOpt_full(:,57)); hold on;
    plot(xOpt_com(1,:), xOpt_com(2,:));
    xlabel('X-axis (m)') ; grid ;ylabel('Y-axis (m)') ; title('Turning Trajectory') ;
    title('x-y plot');
...
for i = 1:13
    hold on
    plot(xDes_all(1,i), xDes_all(2,i),'*')
end
legend('Left Hovershoe','Right Hovershoe','Cassie COM','Obstacle',...
